              ...
              <div className="inline-block w-full max-w-xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                <div className='flex flex-row'>
                  <div className="basis-2/4 m-1">
                    <input className='my-2 p-2 border-2 rounded-md bg-slate-200 disabled:opacity-75' id='nftAddress' onChange={inputHandler} placeholder='NFT Contract Address' />
                    <input className='my-2 p-2 border-2 rounded-md bg-slate-200 disabled:opacity-75' id='tokenId' onChange={inputHandler} onBlur={loadPreview} placeholder='NFT Token ID' />
                    {orderType != 'fixed'?<input className='my-2 p-2 border-2 rounded-md bg-slate-200 disabled:opacity-75' id='startPrice' onChange={inputHandler} placeholder='Start Price' />:null}
                    {orderType != 'auction'?<input className='my-2 p-2 border-2 rounded-md bg-slate-200 disabled:opacity-75' id='buyItNowPrice' onChange={inputHandler} placeholder='Buy it Now Price' />:null}
                    ...
                    <div className='inline-block'>
                      ...
                      <input type="time" className='m-2 p-2 border-2 rounded-md bg-slate-200 disabled:opacity-75' id='endTime' onChange={inputHandler} />
                    </div>
                  </div>
                  ...
                  <div className="basis-2/4 m-1">
                    ...
                    {orderPreview ?
                      <div className='max-w-sm text-center rounded overflow-hidden m-4 shadow-lg'>
                        <h3 className='p-4 font-bold text-xl text-center m-2'>{orderPreview.tokenName}</h3>
                        <img className='px-4' src={orderPreview.tokenImageUri} alt='crypto1' ></img>
                        <div className='px-6 py-4'>
                          <p className='text-gray-700 text-base'>{orderPreview.description}</p>
                        </div>
                      </div>
                      : null}
                  </div>
                ...
                {isProcessing ?
                  <div className='flex'>
                    ...
                    Processing... it can take some time... 2 transactions need to be confirmed in MetaMask
                  </div>
                  :
